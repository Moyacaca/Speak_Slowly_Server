...
def test():
    ...
    add_att = package["add_att"]
    att_param = package["att_param"]
    embeds_param = package["embeds_param"]
    ...
    model = CTC_Model(rnn_param=rnn_param, add_cnn=add_cnn,
                      cnn_param=cnn_param, add_att=add_att, att_param=att_param, embeds_param=embeds_param, num_class=num_class, drop_out=drop_out)
    model.to(device)
