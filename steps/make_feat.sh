...
if [ "$feat_type" != "fbank" ] && [ "$feat_type" != "mfcc" ] &&  [ "$feat_type" != "spectrogram" ]; then
    ...
    compute-$feat_type-feats --config=$feat_config scp,p:$data_dir/wav_sph.scp ark:- | \
        apply-cmvn --norm-vars=true global_${feat_type}_cmvn.txt ark:- ark:- |\
            copy-feats --compress=$compress ark:- ark,scp:$data_dir/$feat_type.ark,$data_dir/$feat_type.scp
fi
