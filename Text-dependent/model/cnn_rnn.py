...
class CTC_Model(nn.Module):
    def __init__(self, add_cnn=False, cnn_param=None, rnn_param=None, add_att=False, att_param=None, embeds_param=None, num_class=39, drop_out=0.1):
        """
        ...
        self.embeds_param = embeds_param
        ...
        if add_cnn:
            ...
            for n in range(len(cnn_layers)):  # cnn_layers have channel kernel_size stride padding
                ...
                try:
                    rnn_input_size = int(math.floor((rnn_input_size+2*padding[1]-kernel_size[1])/stride[1])+1) #如果有使用2D conv輸出的Width(fbank的特徵)會不一樣,參考conv的公式
                except:
                    #if using 1-d Conv
        ...
        # Character_embedding
        num_embeddings = embeds_param["num_embeddings"]
        embedding_dim = embeds_param["embedding_dim"]
        embeds_rnn_hidden_size = embeds_param["embeds_rnn_hidden_size"]
        self.embeds = nn.Embedding(num_embeddings, embedding_dim)
        self.lstm_embeds = nn.LSTM(
            embedding_dim, embeds_rnn_hidden_size, batch_first=True, bidirectional=True)
        # self.score = nn.Linear(rnn_hidden_size*2, rnn_hidden_size*2, bias=False)
        att_num_hiddens = att_param["att_num_hiddens"]
        self.add_attention = AdditiveAttention(
            key_size=2*embeds_rnn_hidden_size,
            query_size=2*rnn_hidden_size,
            num_hiddens=att_num_hiddens, dropout=drop_out)

        if batch_norm:
            self.fc = nn.Sequential(nn.BatchNorm1d(self.num_directions*rnn_hidden_size+2*embeds_rnn_hidden_size),
                                    nn.Linear(self.num_directions*rnn_hidden_size+2*embeds_rnn_hidden_size, num_class, bias=False),)
        else:
            self.fc = nn.Linear(self.num_directions*rnn_hidden_size +
                                2*embeds_rnn_hidden_size, num_class, bias=False)
    ...
    def forward(self, x, x1, visualize=False):
        ...
        if self.add_cnn:
            ...
            # x.size()原先為([batch_size,Cout,sequence_length,feature])
            
            x = x.transpose(1, 2).contiguous()
            # transpose後size為 ([batch_size,sequence_length,Cout,feature])
            sizes = x.size()
            if len(sizes) > 3:
            ...
            # rnn input_shape=(sequence_length, batch_size, input_size)
            x = self.rnns(x)
            ...
            # value
            # rnn output_shape=(batch_size,sequence_length,D∗Hout​) 因為batch_first=true
            x1, _ = self.lstm_embeds(x1)
            x = x.transpose(0, 1).contiguous()
            # 因為add_attention吃的shape為queries:(batch_size, no. of queries, num_hiddens)
            x_attention = self.add_attention(x, x1, x1)
            out1 = torch.cat((x, x_attention), -1)
            x = out1.transpose(0, 1).contiguous()
            seq_len, batch, _ = x.size()
            x = x.view(seq_len*batch, -1)
            x = self.fc(x)
            x = x.view(seq_len, batch, -1) #size()為(seq_len, batch, 43)
    ...
    def compute_wer(self, index, input_sizes, targets, target_sizes):#why do it decode to text directly? 
        # index存的是模型輸出最高機率的index,shape為(batch_size,out_len)
        batch_errs = 0
        batch_tokens = 0
    ...
    def save_package(model, optimizer=None, decoder=None, epoch=None, loss_results=None, dev_loss_results=None, dev_cer_results=None):
        package = {
                ...
                'add_att': model.add_att,
                'att_param': model.att_param,
                'embeds_param': model.embeds_param,
