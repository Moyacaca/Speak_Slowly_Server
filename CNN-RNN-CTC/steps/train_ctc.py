...
import sys
sys.path.append('./')
from utils.TestDecoder import GreedyCTCDecoder
...
parser.add_argument('--conf', default='conf/ctc_config.yaml',
                    help='conf file with argument of LSTM and training')


def run_epoch(epoch_id, model, data_iter, loss_fn, vocab, device, optimizer=None, print_every=20, is_training=True):
    if is_training:
        model.train()
    else:
        model.eval()
    ...
    hypothesis = []
    ground_truth = []
    decoder_greedy = GreedyCTCDecoder(list(vocab.index2word.values()))
    for i, data in enumerate(data_iter):
        ...
        out = model(inputs, trans)  #size()為(seq_len, batch, feature)
        out_len, batch_size, _ = out.size() #out_len每次的長度會不一樣，根據input長度而定
        # input_sizes = (input_sizes * out_len).long()
        input_sizes = torch.full(size=(batch_size,), fill_value=out_len, dtype=torch.long)
        #會需要target_sizes是因為一個batch裡的所有targets都會被擴充成batch裡最長的targets長度
        loss = loss_fn(out, targets, input_sizes, target_sizes)
        loss /= batch_size
        cur_loss += loss.item()
        total_loss += loss.item()
        prob, index = torch.max(out, dim=-1)
        #下面這邊不用decode就直接算wer感覺怪怪的
        ...
        out = out.cpu()
        out = out.transpose(0, 1)#size()為(batch,seq_len, feature)
        for i in range(len(out)):
            hypothesis.append(decoder_greedy(out[i]))
        targets, target_sizes = targets.cpu().numpy(), target_sizes.cpu().numpy()
        for i in range(len(targets)):
            label = [vocab.index2word[num]for num in targets[i][:target_sizes[i]]]
            ground_truth.append(' '.join(label))
        if (i + 1) % print_every == 0 and is_training:
            print('Epoch = %d, step = %d, cur_loss = %.4f, total_loss = %.4f' % (epoch_id, i+1, cur_loss / print_every, total_loss / (i+1)))
            cur_loss = 0
    ...
    for i in np.random.randint(0, len(hypothesis), 2):
        print(f"Target    : {ground_truth[i]}")
        print(f"Prediction: {hypothesis[i]}")
    print("-" * 100)
    average_loss = total_loss / (i+1)
    training = "Train" if is_training else "Valid"
    print("Epoch %d %s done, total_loss: %.4f, total_wer: %.4f" %
          (epoch_id, training, average_loss, WER))
    return 1-WER, average_loss
...
def main(conf):
    ...
    for k, v in conf.items():
        setattr(opts, k, v)
        print('{:50}:{}'.format(k, v))
    tensorboard_path = opts.tensorboard_path
    ...
    train_dataset = SpeechDataset(
        vocab, opts.train_scp_path, opts.train_lab_path, opts.train_trans_path, opts, is_training=True)
    dev_dataset = SpeechDataset(vocab, opts.valid_scp_path, opts.valid_lab_path,
                                opts.valid_trans_path, opts, is_training=False)
    train_loader = SpeechDataLoader(
        train_dataset, batch_size=opts.batch_size, shuffle=opts.shuffle_train, num_workers=opts.num_workers)
    dev_loader = SpeechDataLoader(
        dev_dataset, batch_size=opts.batch_size, shuffle=False, num_workers=opts.num_workers)
    ...
    for layer in range(opts.layers):
        layer_param = [channel[layer], kernel_size[layer],
                       stride[layer], padding[layer]]
    ...
    model = CTC_Model(add_cnn=add_cnn, cnn_param=cnn_param,
                      rnn_param=rnn_param, num_class=num_class, drop_out=drop_out)
    ...
    writer = SummaryWriter(tensorboard_path+'/'+opts.exp_name)
    ...
    #             dict(title=title+" Loss on Dev",
    #                  ylabel='DEV Loss', xlabel='Epoch'),
    ...
    while not stop_train:
        ...
        train_acc, loss = run_epoch(count, model, train_loader, loss_fn, vocab, device,
                                    optimizer=optimizer, print_every=opts.verbose_step, is_training=True)
        loss_results.append(loss)
        acc, dev_loss = run_epoch(count, model, dev_loader, loss_fn, vocab, device,
                                  optimizer=None, print_every=opts.verbose_step, is_training=False)
        print("loss on dev set is %.4f" % dev_loss)
        dev_loss_results.append(dev_loss)
        dev_cer_results.append(acc)
        writer.add_scalar('Loss/train_loss', loss, count)
        writer.add_scalar('Accuracy/train_acc', train_acc, count)
        writer.add_scalar('Loss/dev_loss', dev_loss, count)
        writer.add_scalar('Accuracy/dev_acc', acc, count)
    ...
    conf_save_dir = os.path.join(save_dir, 'ctc_config.yaml')
    f = open(conf_save_dir, "w")
    print(yaml.dump(conf, f))
